...
function GeneratePageContent() {
  ...
  async function handleGenerate() {
    if (!selectedPersonaId || !selectedStage || !problem.trim() || selectedChannels.length === 0)
      return;
    reset();
    await generate(selectedPersonaId, problem.trim(), selectedChannels, selectedStage);
  }
    ...
    <div className="animate-fade-in">
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-4 mb-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            {selectedPersona ? (
              ...
              <div className="flex items-center gap-2">
                ...
                <Link
                  href="/dashboard"
                  className="text-xs font-medium text-coral hover:underline whitespace-nowrap"
                >
                  Create one
                </Link>
              </div>
          ...
          {/* Right: Funnel Stage selector */}
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">
              Select desired funnel stage
            </label>
            <select
              value={selectedStage}
              onChange={(e) => setSelectedStage(e.target.value)}
              className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-700 focus:border-coral focus:ring-1 focus:ring-coral outline-none"
            >
              <option value="" disabled>
        ...
        {/* Action links (shown when persona is selected) */}
        {selectedPersona && (
      ...
      {generatedContent && !isGenerating && (
        <div className="animate-fade-in">
          ...
          {/* Content cards for the selected stage */}
          {activeTab && generatedContent[activeTab] && (() => {
            // Gather content pieces from whichever stage key is present
            const channelData = generatedContent[activeTab] as ChannelContent;
            const activeStageKey = FUNNEL_STAGES.find(
              (s) => channelData[s] && channelData[s].length > 0
            );
            if (!activeStageKey) return null;
            const pieces = channelData[activeStageKey];
            const colors = STAGE_COLORS[activeStageKey];
              ...
              <div>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  {pieces.map((piece: ContentPiece, i: number) => {
                    const key = `${activeTab}-${activeStageKey}-${i}`;
